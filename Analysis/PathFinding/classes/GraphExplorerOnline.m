classdef GraphExplorerOnline < handle
    ...
    methods
        % CONSTRUCTOR
        function obj = GraphExplorerOnline(map, sensor, root_idx)
            obj.map = map;
            obj.sensor = sensor;
        ...
        % prune paths longer than X meters
        function prunePathsLongerThan(obj, meters)
            % array of leaves to be pruned
            leaves_to_prune = [];
            ...
            for n=1:length(obj.all_nodes)
                if obj.all_nodes{n}.isLeaf()
                    ...
                    % if path is too long, throw it out!
                    path_length = obj.map.getPathLength( obj.all_nodes{n}.path );
                    if path_length > meters
                        leaves_to_prune = [leaves_to_prune; n];
                    end
